...
export function Subscription({ username }: Props) {
    ...
    return (
        <View style={styles.container}>
            ...
            <ProfileField
                label="Start Date"
                value={new Date(data.subscription.startDate).toLocaleDateString()}
            />
            <ProfileField
                label="Renewal Date"
                value={new Date(data.subscription.renewalDate).toLocaleDateString()}
            />
            {data.subscription.endDate && (
                <ProfileField
                    label="End Date"
                    value={new Date(data.subscription.endDate).toLocaleDateString()}
                />
            )}
