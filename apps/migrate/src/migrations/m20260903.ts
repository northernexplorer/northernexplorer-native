export const m20260903: string[] = [
	...
	`ALTER TABLE "review" ADD CONSTRAINT "review_conditions_check" CHECK ("conditions" <@ ARRAY['MUD'::text, 'MOSQUITOES'::text, 'DUST'::text, 'SNOW'::text, 'ICE'::text, 'FALLEN_TREES'::text, 'OVERGROWN'::text, 'GARBAGE'::text, 'POISONOUS_PLANTS'::text, 'FLOODED_HIGH_WATER'::text, 'STEEP_CLIMB'::text, 'LOOSE_ROCK'::text, 'LIMITED_PARKING'::text, 'NO_CELL_SERVICE'::text, 'WATER_CROSSING'::text, 'BEAR_ACTIVITY'::text, 'ROUGH_ROAD'::text, 'TICKS'::text, 'BRIDGE_OUT'::text]);`,
