...
export class WeatherController {
    ...
    public static async getInternalWeatherData(lat: number, lon: number): Promise<Record<string, unknown> | null> {
        ...
        const rawResults = (await em.getConnection().execute(query)) as unknown as RawInternalWeatherRow[];
    ...
    public static async getWeatherData(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            const rawResults = (await em.getConnection().execute(query)) as unknown as RawWeatherCacheRow[];
