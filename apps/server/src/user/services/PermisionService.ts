...
export class PermissionService {
	...
	canEditReview({targetId}: {targetId: string}, auth?: AuthContext) {
		...
		if (auth.userId !== targetId) throw new Error("You don't have permission to view this review.");

		return {targetId};
	}

	canEditImage({targetId}: {targetId: string}, auth?: AuthContext) {
		if (!auth?.userId || !auth.email) throw new Error('You must be logged in to access this resource');
		if (auth.roles?.includes(RolesEnum.Admin)) {
			return {targetId};
		}
		if (auth.userId !== targetId) throw new Error("You don't have permission to view this image.");
